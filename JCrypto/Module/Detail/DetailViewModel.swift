...
class DetailViewModel: ObservableObject {
  ...
  @Published var reddditwebsite: String?
  ...
  private func getAdditionalDetails() {
    coinDetailservice.$coinData
      .receive(on: DispatchQueue.main)
      .sink { [weak self] coinData in
        self?.description = coinData?.readableDescription
        self?.website = coinData?.links?.homepage?.first
        self?.reddditwebsite = coinData?.links?.subredditURL
      }
      .store(in: &cancellables)
  }
